...
class BertMatchModel():
    ...
    def train_model(
        ...
        if use_hf_dataset and hf_dataset_name:
            ...
            #                                        self.max_seq_length)
            eval_dataset = BertMatchingTestDataset(self.tokenizer, load_text_matching_train_data(eval_file),
                                                   self.max_seq_length)
        else:
    ...
    def train(
        ...
        if data_parallel:
            ...
            self.model.bert=torch.nn.parallel.DistributedDataParallel(self.model.bert,device_ids=[local_rank],output_device=local_rank)
        if data_parallel:
        ...
        for current_epoch in trange(int(num_epochs), desc="Epoch", disable=False, mininterval=0):
            ...
            if local_rank==0:
                ...
                # results={"eval_spearman":1.1,"eval_pearson":1.1}
                self.model.bert.eval()
                results = self.evaluate(eval_dataloader, output_dir, batch_size=batch_size)
    ...
    def evaluate(self, eval_dataloader, output_dir: str = None, batch_size: int = 16):
        ...
        logger.debug(f"labels: {batch_labels[:10]}",len(batch_labels))
        logger.debug(f"preds:  {batch_preds[:10]}",len(batch_preds))
        # spearman = compute_spearmanr(batch_labels, batch_preds)
        # pearson = compute_pearsonr(batch_labels, batch_preds)
        acc = [p.argmax() == l for p,l in zip(batch_preds,batch_labels)]
        spearman=pearson=sum(acc)/len(batch_labels)
        logger.debug(f"pearson: {pearson}, spearman: {spearman}",acc)
